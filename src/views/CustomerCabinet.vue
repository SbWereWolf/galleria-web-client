<template>
  <div class="all">
    <div> <!-- Профиль пользователя -->
      ...
      <UserContacts text="Посмотреть контакты конкретного художника" @arrow-click="handleArrowClick('artistContact')"/>
...
const activeSectionComponent = computed(() => {
  if (activeSection.value === 'artistContact') {
    return defineAsyncComponent(() => import('@/views/ArtistContactsView.vue'));
  } else if (activeSection.value === 'artists') {
    return defineAsyncComponent(() => import('@/views/LoookArtistView.vue'));
  ...
  } else if (activeSection.value === 'createOrder') {
    return defineAsyncComponent(() => import('@/views/CreatePredzakazArtistView.vue'));
